...
def generate_corpus(verbs, names_masc, names_fem, adjs_masc, adjs_fem, N=10):
    assert(N*10 < (len(verbs)*(len(names_fem)*len(adjs_fem) + len(names_masc)*len(adjs_masc))))# to avoid too much time for doublons resolution
    corpus = set()
    lv = len(verbs)
    lnm = len(names_masc)
    lnf = len(names_fem)
    ...
    while i < N:
        ...
        if masc:
            ...
            word = "{}_un_{}_{}".format(verbs[indexv], names_masc[indexn], adjs_masc[indexa])
        else:
            indexn = randint(0, lnf-1)
            indexa = randint(0, laf-1)
            word = "{}_une_{}_{}".format(verbs[indexv], names_fem[indexn], adjs_fem[indexa])
